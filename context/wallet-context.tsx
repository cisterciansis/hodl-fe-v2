...
export function WalletProvider({ children }: { children: ReactNode }) {
    ...
    // Reason: When the tab regains visibility the wallet extension may have
    // been garbage-collected. Re-enable it so the signer stays valid.
    useEffect(() => {
        const handleVisibilityChange = () => {
            if (document.visibilityState === 'visible' && walletType && selectedAccount) {
                if (!enabledExtensionRef.current?.signer) {
                    refreshSigner()
                }
            }
        }
    ...
    }, [walletType, selectedAccount, refreshSigner])
