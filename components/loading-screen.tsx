...
export function LoadingScreen({
  ...
  // Reason: Safety net — if isReady never fires (e.g. network failure),
  // force the loading screen to dismiss after 8 seconds so the UI is usable.
  useEffect(() => {
  ...
  return (
    <div
      ...
      style={isComplete ? { pointerEvents: "none" } : undefined}
    >
      {/* Scan lines overlay */}
      ...
      <div className="relative z-10 flex flex-col items-center gap-4 sm:gap-6 px-4">
        ...
        <div className="loading-logo-container scale-75 sm:scale-100">
        ...
        {/* Title */}
        <div className="flex flex-col items-center gap-1.5 sm:gap-2 mt-1 sm:mt-2">
          <h1 className="text-[18px] sm:text-[22px] font-normal tracking-tight text-white font-[family-name:var(--font-pixel)] loading-title">
            HODL<span className="ml-2 sm:ml-2.5">Exchange</span>
          </h1>
          <a
            href="https://taomarketcap.com/subnets/118"
            target="_blank"
            rel="noopener noreferrer"
            className="text-[10px] sm:text-[11px] font-medium tracking-[0.25em] uppercase text-blue-400/80 loading-subtitle hover:text-blue-300 transition-colors"
          >
        ...
        {/* Progress bar */}
        <div className="w-48 sm:w-64 mt-2 sm:mt-4">
