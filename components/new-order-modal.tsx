...
export function NewOrderModal({
  ...
  const WS_URL = React.useMemo(() => {
    return getWebSocketNewUrl(selectedAccount?.address ? { ss58: selectedAccount.address } : undefined);
  }, [selectedAccount?.address]);

  const WS_TAP_URL = React.useMemo(() => {
    return getWebSocketTapUrl(selectedAccount?.address ? { ss58: selectedAccount.address } : undefined);
  }, [selectedAccount?.address]);
  ...
  // Reason: If WS reconnects mid-flow (after escrow created), the new UUID
  // won't match the escrow's session. Warn the user and reset escrow state.
  const handleReconnect = React.useCallback(() => {
    if (escrowGenerated && escrowUuidRef.current) {
      setError("Connection lost and re-established. Please create a new escrow.");
  ...
  const handleNext = async () => {
    try {
      ...
      // OFM guardrails: validate order size in TAO equivalent
      const taoEquivalent = formData.type === 2
        ? Number(getTaoForSubmit())
        : (priceForConversion > 0 ? Number(getAlphaForSubmit()) * priceForConversion : 0);
      const [openMax, openMin] = ofm;
      if (taoEquivalent > 0 && taoEquivalent > openMax) {
        throw new Error(`Order size exceeds maximum allowed (${openMax} TAO)`);
      }
      if (taoEquivalent > 0 && taoEquivalent < openMin) {
        throw new Error(`Order size below minimum allowed (${openMin} TAO)`);
      }
      const backendUrl = apiUrl || API_URL;
      ...
      const orderData = buildRecPayload({
        ...
        tao: formData.type === 2 ? Number(getTaoForSubmit()) : 0,
        alpha: formData.type === 1 ? Number(getAlphaForSubmit()) : 0,
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[516px] sm:w-[516px] bg-card dark:bg-background border-border/60 max-h-[90dvh] overflow-y-auto">
        <DialogHeader className="flex flex-row justify-start gap-2 items-center mt-[-10px]">
          <div className="mt-[3px]">
            <DialogTitle className="text-base sm:text-lg">Open Order</DialogTitle>
          </div>
          <ConnectButton />
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-2">
            ...
            <div className="relative flex items-center">
              <Input
                ...
                className="focus-visible:ring-1 focus-visible:ring-blue-500/30 focus-visible:ring-offset-0 focus-visible:border-blue-500/40 pr-14 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              />
              {(transferInputMode === "tao" ? formData.tao : formData.alpha) != null && (
                <span className="absolute right-9 top-1/2 -translate-y-1/2 text-xs text-muted-foreground/50 pointer-events-none font-mono select-none">
                  {transferInputMode === "tao" ? "τ" : "α"}
                </span>
              )}
            ...
            <div className="flex items-center gap-1.5">
              ...
              <Tooltip>
                ...
                <TooltipContent side="top" className="max-w-[260px]">
                  <p>GTC (Good Till Cancel) means the order stays active until you cancel it. Alternatively, pick a specific expiration date.</p>
                </TooltipContent>
              </Tooltip>
            </div>
            <div className="flex gap-2">
          ...
          >
            ...
            <Tooltip>
              ...
              <TooltipContent side="top" className="max-w-[260px]">
                <p>When enabled, your order can be filled in multiple smaller transactions instead of all at once.</p>
              </TooltipContent>
            </Tooltip>
          </div>
