...
import { getWebSocketNewUrl, API_URL } from "@/lib/config";
...
export function FillOrderModal({
  ...
  const handleCreateEscrow = async () => {
    try {
      ...
      // OFM guardrails: validate fill size in TAO equivalent
      const taoEquivalent = fixedValues.type === 2
        ? getTaoForSubmit()
        : (priceForConversion > 0 ? getAlphaForSubmit() * priceForConversion : 0);
      const fillMin = ofm[2];
      if (taoEquivalent > 0 && taoEquivalent < fillMin) {
        throw new Error(`Fill size below minimum allowed (${fillMin} TAO)`);
      }
  ...
  const handleFillOrder = async () => {
    ...
    try {
      ...
      const backendUrl = apiUrl || API_URL;
      const dbRecord = await fetchDbRecord(backendUrl, escrowWallet);
      const fillOrderData = buildRecPayload({
        ...dbRecord,
        uuid: finalUuid,
        origin: order.escrow,
        escrow: escrowWallet,
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[516px] max-h-[90dvh] overflow-y-auto">
        <DialogHeader className="flex flex-row justify-start gap-2 items-center mt-[-10px]">
          <div className="mt-[3px]">
            <DialogTitle className="text-base sm:text-lg">Fill Order</DialogTitle>
          </div>
          <ConnectButton />
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-2">
            ...
            <div className="relative flex items-center">
              ...
              />
              {(transferInputMode === "tao" ? transferTao : transferAlpha) != null && (
          ...
          <div className="grid gap-4 pt-2">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500 dark:text-muted-foreground/80">
                  <span className="normal-case">α</span> Price
                </span>
                <p className="font-mono text-sm mt-1">
