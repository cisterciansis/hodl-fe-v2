...
export const OrderBookRowDetails = React.memo(function OrderBookRowDetails({
  ...
  return (
    <div className="bg-slate-50 dark:bg-muted/30 px-3 sm:px-6 pt-4 sm:pt-6 pb-2 space-y-4 sm:space-y-6 border-t border-slate-200 dark:border-border/50">
          ...
          {order.status === 1 && isOwner && (
            <>
              ...
              <Button
                variant="outline"
                size="sm"
                className="dark:!bg-[#293641] h-8 sm:h-9 gap-1.5 sm:gap-2 text-xs sm:text-sm text-slate-900 dark:text-foreground hover:bg-slate-50 dark:hover:!bg-accent transition-all"
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div>
                  <Button
                    size="sm"
                    className="h-8 sm:h-9 gap-1.5 sm:gap-2 text-xs sm:text-sm bg-gradient-to-b from-blue-500 to-blue-600 hover:from-blue-500 hover:to-blue-700 text-white font-semibold shadow-[0_4px_14px_0_rgba(37,99,235,0.3)] hover:shadow-[0_6px_20px_0_rgba(37,99,235,0.4)]"
      ...
      {filledOrders.length > 0 && (
        <>
          ...
          {/* Mobile: card layout */}
          <div className="!my-2 sm:hidden space-y-2">
            {filledOrders.map((filledOrder, index) => {
              const isClosed = filledOrder.status === 3;
              const orderTypeLabel = getOrderType(filledOrder.type);
              const uniqueKey = `${filledOrder.uuid}-${filledOrder.escrow}-${index}`;
              ...
              return (
                <div key={uniqueKey} className={`rounded-md border border-slate-200 dark:border-border/40 p-3 space-y-2 ${flashClass}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Badge variant={orderTypeLabel === "Buy" ? "outline" : "secondary"} className={`font-medium text-xs ${orderTypeLabel === "Buy" ? "text-emerald-600 border-emerald-200 bg-emerald-50 dark:bg-emerald-950/30 dark:border-emerald-800 dark:text-emerald-400" : "text-rose-600 border-rose-200 bg-rose-50 dark:bg-rose-950/30 dark:border-rose-800 dark:text-rose-400"}`}>{orderTypeLabel}</Badge>
                      <Badge variant="outline" className="font-medium text-xs text-gray-600 dark:text-gray-400 border-gray-200 dark:border-border">{isClosed ? "Closed" : "Filled"}</Badge>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="font-mono text-xs text-muted-foreground" title={filledOrder.escrow}>{formatWalletAddress(filledOrder.escrow)}</span>
                      <button onClick={(e) => { e.stopPropagation(); copyToClipboard(filledOrder.escrow, "filledEscrow", uniqueKey); }} className="text-muted-foreground hover:text-foreground transition-colors flex-shrink-0 opacity-60 hover:opacity-90" title="Copy escrow address">
                        {copiedFilledEscrowIds.has(uniqueKey) ? <CheckIcon className="h-3 w-3 text-emerald-500" /> : <Copy className="h-3 w-3" />}
                      </button>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-sm">
                    <div>
                      ...
                      <div className="font-mono tabular-nums">{formatTao(Number(filledOrder.tao ?? 0))}</div>
                    </div>
                    <div>
                      <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Alpha</div>
                      <div className="font-mono tabular-nums">{formatNumber(Number(filledOrder.alpha ?? 0))}</div>
                    </div>
                    <div className="text-right">
                      ...
                      <div className="font-mono tabular-nums">{filledOrder.price != null && Number(filledOrder.price) > 0 ? formatPrice(Number(filledOrder.price)) : formatPrice(filledOrder.stp || 0)}</div>
                    </div>
                  </div>
                </div>
              );
