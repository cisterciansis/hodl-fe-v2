...
/**
 * Parse a date string/Date/number into a UTC Date, handling backend formats.
 * Returns null if invalid.
 */
function parseUTCDate(date: string | Date | number | null | undefined): Date | null {
...
function SortableHeader<TData>({
  ...
  return (
    <button
      type="button"
      className={`flex items-center gap-1 hover:text-foreground transition-colors select-none uppercase ${className}`}
...
export const columns = (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const taostatsUrl = `https://taomarketcap.com/subnets/${asset}`;
      return (
          ...
          <a
            ...
            title={`View subnet ${asset} on TaoMarketCap`}
            onClick={(e) => e.stopPropagation()}
          >
            <ExternalLink className="h-3.5 w-3.5" />
          </a>
  ...
  },
  {
    ...
    header: ({ column }) => (
        ...
        <SortableHeader column={column} label={<><span className="normal-case">α</span> Price</>} />
      </div>
    ),
    cell: ({ row }) => {
